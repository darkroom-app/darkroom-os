...
# when a project (or a new kadar type on an existing project) is added in
# the app.
#
...
# leaves it as-is (see sync.log for anything that looks like it might be
# a stale/renamed project, and rename it by hand if that's actually what
# happened).
#
# All the naming logic (which kadar type maps to which subfolder) lives in
# the datacenter-folder-plan Edge Function, not here — this script's only
# job is to turn whatever that function returns into folders on disk.
#
...
#
# SETUP (one-time):
#   1. Copy this file to the datacenter server, e.g.
#      D:\DarkroomSync\darkroom-datacenter-sync.ps1
#   2. Fill in the two placeholder values below.
#   3. Register a Task Scheduler task (see bottom of this file) that runs
#      it once or a few times a day — no urgency, unlike the RenderFlow
#      bridge.
#
...
try {
    ...
    foreach ($p in $plan.projects) {
        ...
        foreach ($sub in $p.rendererSubfolders)  { Ensure-Folder (Join-Path $root "Renderi\$sub") }
        foreach ($sub in $p.maxSubfolders)       { Ensure-Folder (Join-Path $root "Max\$sub") }
    }
...
1. Open Task Scheduler (search "Task Scheduler" in Start menu) on the
   datacenter server itself.
...
5. Actions tab -> New:
   ...
   - Add arguments:
       -NoProfile -ExecutionPolicy Bypass -File "D:\DarkroomSync\darkroom-datacenter-sync.ps1"
